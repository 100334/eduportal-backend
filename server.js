...
app.get('/api/teacher/reports', authenticateToken, async (req, res) => {
  try {
    console.log('📋 Fetching reports for teacher');
...
app.get('/api/teacher/attendance', authenticateToken, async (req, res) => {
  try {
    console.log('📅 Fetching attendance records for teacher');
...
app.get('/api/teacher/attendance/date/:date', authenticateToken, async (req, res) => {
  try {
...
app.get('/api/teacher/attendance/summary/:classId', authenticateToken, async (req, res) => {
  try {
    const { classId } = req.params;
    const { term = 1, year = new Date().getFullYear() } = req.query;
    ...
    const summary = (learners || []).map(learner => {
      const learnerAttendance = (attendance || []).filter(a => a.learner_id === learner.id);
      const presentCount = learnerAttendance.filter(a => a.status === 'present' || a.status === 'late').length;
...
app.listen(PORT, () => {
  ...
  console.log('\n📋 Teacher API Endpoints:');
  console.log('   GET    /api/teacher/dashboard/stats ✅ (NEW)');
  console.log('   GET    /api/teacher/learners');
  console.log('   GET    /api/teacher/learners/:classId');
  console.log('   POST   /api/teacher/learners');
  console.log('   PUT    /api/teacher/learners/:id');
  console.log('   DELETE /api/teacher/learners/:id');
  console.log('   GET    /api/teacher/reports ✅ (NEW)');
  console.log('   POST   /api/teacher/reports ✅ (NEW)');
  console.log('   DELETE /api/teacher/reports/:id ✅ (NEW)');
  console.log('   GET    /api/teacher/attendance ✅ (NEW)');
  console.log('   POST   /api/teacher/attendance');
  console.log('   GET    /api/teacher/attendance/date/:date ✅ (NEW)');
  console.log('   GET    /api/teacher/attendance/summary/:classId ✅ (NEW)');
  console.log('   GET    /api/teacher/subjects/:classId');
